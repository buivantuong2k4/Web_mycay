...
<div class="danhmuc"> 	
  ...
  <div class="box">
    ...
    ?>
    <div class="box-item">
      <a href="?quanly=shop&action=chitiet&idsp=<?php echo (int)$sp['id_sanpham'] ?>">
        <div class="box-img"><img class="img img-responsive" width="100%" src="Public/img/<?php echo htmlspecialchars($sp['hinhanh'] ?? '') ?>" alt="<?php echo htmlspecialchars($sp['tensanpham'] ?? '') ?>"></div>
        <p class="title_product"><?php echo htmlspecialchars($sp['tensanpham'] ?? '') ?></p>
        <p class="price_product">Giá : <?php echo number_format(floatval($sp['gia'] ?? 0),0,',','.') ?> VNĐ</p>
      </a>
      <?php if(!empty($sp['soluong']) && $sp['soluong']>0){ ?>
        <button class="add_cart" value="<?php echo (int)$sp['id_sanpham'] ?>">+</button>
      <?php } ?>
    </div>
    <?php }
    } ?>
              ...
              <p style="margin:10px 0">Trang hiện tại : <?php echo $page ?>/<?php echo $trang ?> </p>
              <ul class="list_trang">
					<?php
